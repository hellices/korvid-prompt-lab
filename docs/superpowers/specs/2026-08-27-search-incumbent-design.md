...
Treat the existing campaign `champion` as the search incumbent. The first
successful, non-systemic SEARCH outcome establishes its first real score,
regardless of whether the optimizer returned the same fingerprint or a
different partially unsafe candidate. It resets stagnation because the
campaign has obtained its baseline observation.
...
state: tier 0, stage 0, seed 0, zero metric calls, and zero elapsed search
progress. No schema migration is required; a fresh campaign lineage is still
...
- The first real unsafe candidate replaces the synthetic initial score.
